...
use bevy::{
    ...
    color::palettes::{basic::YELLOW, css::BLUE},
    core_pipeline::core_2d::{CORE_2D_DEPTH_FORMAT, Transparent2d},
    math::{FloatOrd, ops},
    mesh::{Indices, MeshVertexAttribute, VertexBufferLayout},
    prelude::*,
    render::{
        Extract, Render, RenderApp, RenderStartup, RenderSystems, extract_component::{ExtractComponent, ExtractComponentPlugin}, mesh::RenderMesh, render_asset::RenderAssets, render_phase::{
        ...
        }, sync_world::{MainEntityHashMap, RenderEntity}, view::{ExtractedView, RenderVisibleEntities, ViewTarget}
    },
    sprite_render::{
...
fn main() {
    let mut app = App::new();
    app.add_plugins((DefaultPlugins, ColoredMesh2dPlugin));
    app.add_systems(Startup, spawn_octagon);
    app.run();
}
...
fn spawn_octagon(
    ...
    // We need 1 center vertex + 8 outer vertices = 9 vertices total
    let mut v_pos = vec![[0.0, 0.0, 0.0]]; // Index 0: Center
    for i in 0..8 {
        // An octagon has 8 sides, so we divide a full circle (2 * PI) by 8.
        let a = i as f32 * (PI / 4.0); 
        let radius = 200.0;
        v_pos.push([radius * ops::cos(a), radius * ops::sin(a), 0.0]);
    }
    octagon.insert_attribute(Mesh::ATTRIBUTE_POSITION, v_pos);

    // 2. Assign Colors
    // Center vertex: Blue with 0.0 alpha (fully transparent)
    let transparent_blue = LinearRgba::new(0.0, 0.0, 0.0, 0.0).as_u32();
    let mut v_color: Vec<u32> = vec![transparent_blue];

    // Outer vertices: Solid blue (1.0 alpha)
    // Don't forget to update your imports to use `bevy::color::palettes::basic::BLUE`
    let solid_blue = LinearRgba::from(BLUE).as_u32();
    v_color.extend_from_slice(&[solid_blue; 8]);
    
    octagon.insert_attribute(
        MeshVertexAttribute::new("Vertex_Color", 1, VertexFormat::Uint32),
        v_color,
    );

    // 3. Define Triangles (Indices)
    // Connect the center (0) to the outer edges in counter-clockwise winding order.
    // The last triangle connects vertex 1 and 8 to close the shape.
    let mut indices = Vec::new();
    for i in 1..8 {
        indices.extend_from_slice(&[0, i, i + 1]);
    }
    // The last triangle connects vertex 8 back to vertex 1 to close the shape.
    indices.extend_from_slice(&[0, 8, 1]);
    
    octagon.insert_indices(Indices::U32(indices));

    // Spawn the mesh and camera
    commands.spawn((
        ColoredMesh2d,
...
impl Plugin for ColoredMesh2dPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins(ExtractComponentPlugin::<ColoredMesh2d>::default());
...
pub fn extract_colored_mesh2d(
    ...
    for (entity, render_entity, view_visibility, transform, handle) in &query {
        ...
        let transforms = Mesh2dTransforms {
            // Notice the parentheses: we take a reference to the result of affine() 
            // before calling into()
...
pub fn queue_colored_mesh2d(
    ...
    for (visible_entities, view, msaa) in &views {
        ...
        let visible_entities = visible_entities.get::<Mesh2d>() else {
            continue;
        };
        for (render_entity, visible_entity) in visible_entities.iter() {
